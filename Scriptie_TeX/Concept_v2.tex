...
According to a heuristic based on the BN structure and correpsonding CPTs, sampling methods generate a bunch of states of a BN. These configurations, or samples, could be used to do inference. The next example illustrates the key concept of all approximate sampling techniques.
\begin{example}\label{sampling}
Consider the BN in Figure \ref{rainBN}. Let \rr be our variable of interest and suppose $Y_1, \ldots , Y_6$ are independent and identically distributed (i.i.d.) samples of $(R, S, G)$ created by a certain sampling heuristic,
\begin{multicols}{2}
\begin{enumerate}
\item $Y_1 = ( R_1 = \rF, S_1 = \sT,  G_1 = \gT)$
\item $Y_2 = ( R_2 = \rT, S_2 = \sF,  G_2 = \gT)$
\item $Y_3 = ( R_3 = \rF, S_3 = \sT,  G_3 = \gT)$
\item $Y_4 = ( R_4 = \rF, S_4 = \sT,  G_4 = \gF )$
\item $Y_5 = ( R_5 = \rT, S_5 = \sT,  G_5 = \gT)$
\item $Y_6 = ( R_6 = \rF, S_6 = \sF,  G_6 = \gF)$.
\end{enumerate}
\end{multicols}
...
\begin{align*}
P(R = \rT) = \E[ \mathds{1}_{R = \rT} ] \approx \sum_{t=1}^6 \mathds{1}_{R_t = \rT} = \frac{2}{6},
\end{align*}
where $\mathds{1}_{R = \rT}$ is the indicator function of the event $R = \rT$.
\end{example}
In general, for a variable of interest $Y$ with $T \in \N$ (not te be confused with T  for true) i.i.d. samples the probability of $Y = y$ can be approximated by 
\begin{align*}
P(Y = y) = \E[ \mathds{1}_{Y=y} ] \approx \sum_{t=1}^T \mathds{1}_{Y_t = y}.
\end{align*}
A plethora of techniques have been develop to create samples $Y_1, \ldots Y_T$. Again, discussing all those methods here in detail is beyond the scope of this thesis. For an overview of approximate inference methods and its limitations we refer to \cite{koller2009probabilistic, nielsen2009bayesian}. In the next subsection, we focus on how samples can be generated by using Markov chain Monte Carlo simulations. 

\subsection{Markov chain Monte Carlo sampling}
In contrast to Example \ref{sampling}, suppose that we do not create samples $Y_t$ i.i.d. for all $t \geq 1$. But we create  sample $Y_{t+1}$ by tweaking the state of sample $Y_{t}$ according to a certain heuristic. Repeating this process yields the Markov chain $(Y_t)_{t \in \N_0}$. Repeating this often -- for $t = 1, \ldots T$ with large $T$  -- this process is called \textit{Markov chain Monte Carlo (MCMC) sampling}. 
...
\end{align}
\end{definition}
In the next example, we illustrate one such a heuristic that guarantees regularity and reversibility of the Markov chain.
\begin{example}[Metropolis sampling]
Consider the BN in Figure \ref{rainBN}. Suppose we have the initial state 
\begin{align*}
Y_0 = (R = \rT, S = \sF, G = \gT),
\end{align*}
shortened $(\rT, \sF, \gT)$ and no evidence is available. Based on $Y_0$, according to the fixed ordening $(R, S, G)$, we can change 
\end{example}
Another approach to create Markov chains that converge to the desired posterior distribution is called Gibbs sampling. We expose this method in the next subsection.

\subsection{Gibbs sampling}
\begin{example}[Gibbs sampling]
Consider the BN in Figure \ref{rainBN}, suppose we have the initial state 
\begin{align*}
Y_0 = (R_0 = \rT, S_0 = \sT, G_0 = \gT)
\end{align*}
and no evidence is available. For each Gibbs iteration, we resample all unobserved variables, one in a time, in the predetermined (topological) order. Thus, we first sample $R_1$ from the distribution $P(R_1 | S_0 = \sT, G_0 = \gT)$, which could be made explicitly
\begin{align*}
P(R_1 = \rT | S_0 = \sT, G_0 = \gT) &= \frac{P(\rT, \sT,  \gT)}{P(\sT, \gT)} = \frac{P(\gT | \rT, \sT) P(\sT | \rT) P(\rT)}{\sum_{\rr \in {\{T,F\}}}P(\rr, \sT, \gT)} \\
&= \frac{0.99 \cdot 0.01 \cdot 0.2}{0.00198{\rT, \sT, \gT} + 0.288_{\rF, \sT, \gT}} \approx 0.0068.
\end{align*}
Hence $P(R_1 = \rF | S_0 = \sT, G_0 = \gT) \approx 0.9932$. Having sampled $R_1 = \rF$, we now continue to resample $S_1$ from the distribution $P(S_1 | R_1 = \rF, G_0 = \gT)$, obtaining for example $S_1 = \sT$. Finally, we sample $G_1$ from $P(G_1 | R_1 = \rF, S_1 = \sT)$, resulting in $\gT$. Therefore, the result of the first iteration of Gibbs sampling is the sample $Y_1 = (R_1 = \rF, S_1 = \sT, G = \gT)$. This process repeats.
\end{example}
\begin{center}
\begin{figure}[h!]
\centering
...
\end{subfigure}\hfill%
\begin{subfigure}{.33\linewidth}
\includegraphics[width=1.1\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Alarm/Prune/alarm_hist_num_samples.png}
