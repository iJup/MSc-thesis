...
In this subsection, we show how relatively short simulations -- $100$ simulations of $25.000$ samples -- could be used to determine the rate of convergence (ROC) carefully. \\
...
\begin{align*}
g(t) = {\sigma}^2(t) \sqrt{t} = \alpha(1+ \beta t^{-\delta})\ .
\end{align*}
One should note that if we plot $g(t)$ in terms of $t^{-\delta}$, for certain $\delta$, $g(t)$ becomes approximately a linear function. We learn how to choose $\delta$ by doing. Once, we have found an approximate linear plot - like Figure \ref{fig2}(b) -- where we have chosen $\delta = 0.90$ -- we could interpret $\alpha$ as the point of intersection with the y-axis. Hence, for the bunch of Metropolis samples displayed in Figure \ref{fig1}(a) we determine that the proportionality constant of the ROC is $\alpha = 0.40$.
