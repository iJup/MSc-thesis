...
type=symbols,
name={\ensuremath{V}},
description={set of predetermined fixed size, contains feasible states generated by HFS}
}
...
\usepackage[hmarginratio=1:1, left=25mm, top=25mm, bottom=35mm]{geometry}
\allowdisplaybreaks
...
The task of answering questions as \textit{what is $P(R = \text{r}_\text{T} | G = \text{g}_\text{T})$?} is called \textit{inference}. Below, we describe how one could answer such queries by computing the exact probability. \\
...
Before elaborating on sampling inference methods, we first discuss how to sample from a finite discrete distribution.
\begin{definition}[Sampling from a categorical distribution]
From section \ref{sec:BN} we know that the distribution of every $X_i$ in a BN given the state of its parents is denoted by 
\begin{align*}
P(X_i | \text{Pa}(X_i) = \bfx ).
\end{align*}
Suppose $Val(X_i) = M$, then each of the $M$ possible states of $X_i$ is attained with probability $p_1, \ldots, p_M$, where $\sum_{m=1}^M p_m = 1$. Note that these probabilities $p_m$ are the column CPT-entries of variable $X_i$. Sampling from such a distribution can be done by partitioning the interval $[0,1)$ in $M$  smaller intervals (corresponding to $Val(X_i)$) of the form
\begin{align*}
[0, p_1), [p_1, p_1 + p_2), [p_1 + p_2, p_1 + p_2 + p_3), \ldots , [1-p_M, 1).
\end{align*}
Then, a random number generator could be used to generate a random number $r_0$ in the interval $[0,1)$. The unique smaller interval in which $r_0$ is found yields the sampled state.
\end{definition}
...
\resizebox{0.5\columnwidth}{!}{
    \begin{tabular}{|c||c|c|c|c|} \hline
	& {\small $\text{r}_\text{T},\ \text{s}_\text{T}$ }& {\small $\text{r}_\text{T},\ \text{s}_\text{F}$ } & {\small $\text{r}_\text{F},\ \text{s}_\text{T}$ } & {\small $\text{r}_\text{F},\ \text{s}_\text{F}$ } \\ \hline \hline
	{\small $ \text{g}_\text{T}$ } & $\star$ & $\star$ & $\star$ & 0  \\ \hline
	{\small $\text{g}_\text{F}$ } & $\star$ & $\star$ & $\star$ & $\star$ \\ \hline
\end{tabular}
}.
...
\begin{example}
Without loss of generality, we consider the initial state $\bfx^{(0)} = (0,0)$. Following the \textit{pruning} procedure we obtain
\begin{itemize}
\item $\C_\bfx = \{ A(1), B(1) \}$.
...
\includegraphics[width=1.2\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/Metropolis/delta_dev/d_04_new}
\end{subfigure}\hfill%
\begin{subfigure}{.3\linewidth}
\includegraphics[width=1.2\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/Metropolis/delta_dev/d_05_new}
\end{subfigure}\hfill%
\begin{subfigure}{.3\linewidth}
\includegraphics[width=1.2\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/Metropolis/delta_dev/d_06_new}
\end{subfigure}\hfill%
\begin{subfigure}{.3\linewidth}
\includegraphics[width=1.2\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/Metropolis/delta_dev/d_07_new}
\end{subfigure}\hfill%
\begin{subfigure}{.3\linewidth}
\includegraphics[width=1.2\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/Metropolis/delta_dev/d_08_new}
\end{subfigure}\hfill%
\begin{subfigure}{.3\linewidth}
\includegraphics[width=1.2\columnwidth]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/Metropolis/delta_dev/d_09_new}
\end{subfigure}\hfill%
\vspace{0.75pc}
...
%Therefore, we only consider a subset of the interval $[0,1]$ to determine the proportionality constant $\alpha$. In Figure \ref{fig2}(b) and \ref{fig3} we took the interval $[0,0.5]$. 
...
\centering
\captionsetup{width = 0.9\textwidth}
\includegraphics[width = 0.68\textwidth]{/Users/jurriaan/Documents/LaTeX/MSc_thesis_UU/Scriptie_TeX/Figure1}
...
In other words, where the number of feasible states of a normal non-pruned BN increases as the number of variables and parameters increases, we expect an exponential blow up of $|S_{\C_\bfx^{\text{np}}}|$ to be the main reason of excessive time-consumption. To illustrate this, we display in Figure \ref{results4} a histogram of the number of feasible states ($|S_{\C_\bfx^{\text{np}}}|$) generated during $2.400$ iterations of \ps on the Asia, Alarm and Win95pts BNs without any available evidence.
\begin{figure*}[h!]
\centering
\captionsetup[subfigure]{justification=centering}
...
\label{results4}
\end{figure*}
We choose these $2.400$ iterations random from the $25.000$ generated samples. We observe a maximum of respectively $16$, $2.218$ and $34.125$ number of feasible states found during one iteration.\\

We observe from the histograms that for the medium and large BN the maximum value for $|S_{\C_\bfx^{\text{np}}}|$ is an extreme outlier. For the Alarm and Win95pts BN without evidence, the maximum set sizes are $2.218$ and $34.125$, respectively $35$ and $83$ times the average set size and where median set sizes are $34$ and $50$ respectively. Therefore, for a significant part the set with the maximum number of feasible states will be responsible for the excessive computation time of \ps on large BNs.\\
...
\label{results6}
\end{figure*}
Note that, due to the presence of evidence, more CPT-entries are fixed during the generation of feasible states. Hence, the number of feasible states generated during one iteration and the maximum set size of $|S_{\C_\bfx^{\text{np}}}|$ decreases. Though, the maximum set sizes still stand far apart from the average set size $1.95$, $22.02$, $176.77$ and median values $2$, $12$, $108$ respectively.\\ 

For the Grid $3 \times 3$, $5 \times 5$ and $8 \times 8$ BNs we find average set sizes of $3.11$, $17.02$ and $794.02$ feasible states. With set sizes respectively $4.5$, $7,6$ and $10.27$ times bigger than average and median values $3$, $14$ and $609.5$, we conclude that the maximum number of feasible states is less scattered.\\

Now, we know that the excessive time consumption of \ps is caused by pruned BNs with large amounts of feasible states. Enumeration of those feasible states to choose completely uniformly the next sample is rather time intensive. In order to improve \ps one could consider to curb the enumeration of all feasible states, i.e. to limit the set size $|S_{\C_\bfx^{\text{np}}}|$. This approach could be interpreted as a trade-off between uniformity and computational effort. We discuss this concept known as \textit{hybrid forward sampling} further in the next section.
...
The existing \ps method uses regular forward sampling to find feasible states in the pruned BN. As discussed in section \ref{sec:gen_init}, regular forward sampling could generate biased samples. To avoid this bias, we consider \gls{HFS} -- as defined in definition \ref{def:hfs} -- and this time we limit the set size $|S_{\C_\bfx^{\text{np}}}|$ of generated feasible states to a predetermined fixed. Subsequently, from this set $\gls{V}$ with candidate samples we choose randomly a next sample (state of the BN). The size of $V$ can be interpreted as a trade-off between uniformity and computational effort. \\

Based on the acceptable maximum set size $|S_{\C_\bfx^{\text{np}}}|$ (Figure \ref{results4}(a)-\ref{results6}(a)) and the modest time consumption of \ps (Table \ref{time-table}) on small BNs, we consider it unnecessary to curb the set size $V$ for these type of BNs at all. For medium sized BNs, we conduct experiments in terms of accuracy and time-consumption for $|V| = 10, 100$. For large sized BNs and $|V| = 10, 100, 1000$, we conduct experiments for the same performance indicators. Since the set size $|V|$ represents a subset of the whole state space, we expect more near-uniform samples to be drawn as more feasible states are available in the set $V$. Hence, we expect increasing accuracy of \ps using the HFS approach as $|V|$ increases. In terms of accuracy, the results of the \ps technique using HFS for different set sizes of $V$, is presented in Figure \ref{results7}.\\

We see that \ps that uses the HFS approach is consistently inferior to Gibbs-, Metropolis- and regular \ps on all type of medium and large BNs. Though, some interesting insights could be deduced from these results. A first observation is that on all BNs displayed in Figure \ref{results7}, it is remarkable that \textit{prune HFS} for $V = 10, 100$ and/or $1000$ all show the same performance. On the Win95pts BN with 25\% evidence available, the HFS with $V = 10$ even performs better than HFS using $V = 100$ and $1000$. This is in contrast to our expectations. Apparently, the trade-off between computational effort and uniformity behaves different than we consider it to be.
\begin{figure*}[h!]
\centering
\begin{subfigure}{.5\linewidth}
\includegraphics[height=5.3cm]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Alarm/alarm_ahd5_25000.png}
\caption{Alarm 0\% determinism}%
\label{alarm_ev}%
\end{subfigure}\hfill%
\begin{subfigure}{.5\linewidth}
\includegraphics[height=5.3cm]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Win95pts/win95pts_ahd5_25000.png}
\caption{Win95pts 0\% determinism}%
\label{win95pts_ev}%
\end{subfigure}
\begin{subfigure}{.5\linewidth}
\includegraphics[height=5.3cm]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Alarm_ev25/alarm_ev_ahd5_25000.png}
\caption{Alarm 25\% determinism}%
\label{grid_3x3}%
\end{subfigure}\hfill%
\begin{subfigure}{.5\linewidth}
\includegraphics[height=5.3cm]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Win95pts_ev25/win95pts_ev_ahd6_25000.png}
\caption{Win95pts 25\% determinism}%
\label{grid_5x5}%
\end{subfigure}
\begin{subfigure}{.5\linewidth}
\includegraphics[height=5.3cm]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_5x5/grid_5x5_ahd5_25000.png}
\caption{Grid 5x5 50\% evidence}%
\label{grid_3x3}%
\end{subfigure}\hfill%
\begin{subfigure}{.5\linewidth}
\includegraphics[height=5.3cm]{/Users/jurriaan/Documents/MScWiskunde/20180907_TNOdrive/Bayesian_network_repository/xdsl-benchmark-GeNIe-files/Grid_8x8/grid_8x8_ahd6_25000.png}
\caption{Grid 8x8 50\% evidence}%
\label{grid_5x5}%
\end{subfigure}
\vspace{0.75pc}
\caption{consistent underperformance of \ps using the \gls{HFS} approach. HFS allows \ps to construct a set $V$ of predetermined fixed size ($10$, $100$ and/or $1000$) of generated feasible states from the pruned BN in order to select (randomly) a state from $V$ as the new state of the BN for the next iteration.}
\label{results7}
